"""
This rule is the fisrt filter : selection of genes of interest only (liste)
"""
rule filter_gene:
    input:
        "vcf/{sample}.vcf"
    output:
        "filter_vcf/{sample}.vcf"
    message:
    ...
    log:
        "logs/filter_1/{sample}.vcf.log"

    shell:
         "(java -jar ~/snpEff/SnpSift.jar extractFields -s ',' -e '.' {input} 'ANN[0].GENE' 'ANN[0].GENEID' 'CHROM' 'POS' 'ANN[0].EFFECT' 'VARTYPE' \
           'REF' 'ALT' 'ANN[0].HGVS_C' 'ANN[0].HGVS_P' 'ANN[0].TRID' 'ANN[0].RANK' 'ANN[0].CDNA_POS' 'ANN[0].CDS_POS' 'ANN[0].AA_POS' \
